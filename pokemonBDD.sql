...
BEGIN
    ...
    IF comprobar = 0 THEN
		DELETE FROM equipo WHERE ID_POKEMON = new.ID_POKEMON;
	END IF;
END $$
DELIMITER ;
...
BEGIN
	...
	SET comprobar = (SELECT EQUIPO FROM POKEMON WHERE ID_POKEMON = new.ID_POKEMON);
    IF comprobar = 0 THEN
		INSERT INTO equipo(ID_POKEMON,OBJETO) VALUES (new.ID_POKEMON, (SELECT obj.NOMBRE FROM pokemon pok LEFT JOIN objetos obj ON pok.OBJETO = obj.ID_OBJETO WHERE ID_POKEMON = new.ID_POKEMON));
	END IF;
END $$
DELIMITER ;
